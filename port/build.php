...
function buildCompleteCSS(){
	...
	if(in_array (true, $completeCSS)){
		...
		system("sass --style=compressed --no-source-map build/xui.complete.css > bin/css/xui.complete.min.css");
...
function buildCompleteJS(){
	...
	if(in_array (true, $completeJS)){
		...
		foreach($completeJS as $js => $isBaked){
 		...
 		system("uglifyjs -c -m -o bin/js/xui.complete.min.js --comments \"/^!/\" build/xui.complete.js");
