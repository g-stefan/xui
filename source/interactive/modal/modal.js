...
XUI.Modal.activate = function(elId) {
	...
	if (el) {
		...
		this.setDeactivateEventListener(XUI.Element.getByClassNameFirst(el, "xui-modal_modal-content"));
...
XUI.Modal.deactivateAnimationFinish = function() {
	...
	if (this.list[index].elActive) {
		...
		if (!this.list[index].elInitialWidthFirstTime) {
			...
			var el = XUI.Element.getByClassNameFirst(this.list[index].elActive, "xui-modal_modal-content");
			if (el) {
...
XUI.Modal.onResize = function() {
	...
	if (this_.list[index].elActive) {
		var el = XUI.Element.getByClassNameFirst(this_.list[index].elActive, "xui-modal_modal-content");
		if (el) {
...
XUI.Modal.init = function() {
	...
	for (var elIndex = 0; elIndex < elList.length; ++elIndex) {
		var el = XUI.Element.getByClassNameFirst(elList[elIndex], "xui-modal_modal-close-button");
		if (el) {
			(function(elSuper, el) {
