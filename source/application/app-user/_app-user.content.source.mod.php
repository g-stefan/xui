<?php
/*
// XUI
// Copyright (c) 2017-2024 Grigore Stefan <g_stefan@yahoo.com>
// MIT License (MIT) <http://opensource.org/licenses/MIT>
// SPDX-FileCopyrightText: 2017-2024 Grigore Stefan <g_stefan@yahoo.com>
// SPDX-License-Identifier: MIT
*/
?>
<div class="xui-app-user <?php echo $appUserMod; ?>">
	<div class="xui-app-user_content">
		<div class="xui-app-user_image">
			<div class="xui-app-user_image_img"></div>
		</div>
		<div class="xui-app-user_info">
			<div class="xui-app-user_name">John Doe</div>
			<div class="xui-app-user_position">Manager</div>
		</div>
	</div>
</div>
