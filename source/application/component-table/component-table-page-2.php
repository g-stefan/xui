<?php
/*
// XUI
// Copyright (c) 2017-2024 Grigore Stefan <g_stefan@yahoo.com>
// MIT License (MIT) <http://opensource.org/licenses/MIT>
// SPDX-FileCopyrightText: 2017-2024 Grigore Stefan <g_stefan@yahoo.com>
// SPDX-License-Identifier: MIT
*/
?>
<div class="xui-page --A4 --presentation shadow-md mx-auto">

<div class="text-lg border-b-1 border-xui-line">
Component - Table
</div>
<div class="clear-both h-4"></div>

<!-- item -->
<form>
<div class="xui-component-table">
	<div class="xui-app-toolbar">
		<div class="xui-app-toolbar_content">
			<div class="xui-button --transparent xui-effect-ripple --primary --icon">
				<i class="lucide-grip"></i>
			</div>
			<div class="xui-button --transparent xui-effect-ripple --primary --icon">
				<i class="lucide-shapes"></i>			
			</div>
		</div>
	</div>
	<div class="xui-app-toolbar --compact --wide">
		<div class="xui-app-toolbar_content-left">
			<div class="xui-form-input-group">
				<input type="text" name="search" value=""></input>
				<button type="button" name="search-button-search"><i class="lucide-search"></i></button>
				<button type="button" name="search-button-reset"><i class="lucide-x"></i></button>
			</div>
		</div>
		<div class="xui-app-toolbar_content-right">			
			<div class="xui-button --small --transparent --secondary --icon-left --toolbar xui-effect-ripple">
				<i class="lucide-filter"></i>
				<span>Filter</span>
			</div>
		</div>		
	</div>
	<div class="xui-component-table_table">
	        <table class="xui-table">
			<thead>
				<tr>
					<th class="xui-component-table_select">
						<div class="xui-form-checkbox --box">
							<input type="checkbox" id="checkbox-item-box-xx-2" name="checkbox-item-box-xx-2" value="checkbox-value"></input>
							<label for="checkbox-item-box-xx-2"></label>
						</div>
					</th>
					<th class="xui-component-table_sort --action">
						Firstname
						<div class="xui-button --transparent xui-effect-ripple --primary --icon --small --size-xy28">
							<i class="lucide-done-all"></i>
						</div>
						<div class="xui-button --transparent xui-effect-ripple --secondary --icon --small --size-xy28 --circle">
							<i class="lucide-chevron-down"></i>
						</div>
					</th>
					<th>
						Lastname
					</th>
					<th>
						Age
					</th>
					<th>
						Action
					</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="xui-component-table_select">
						<div class="xui-form-checkbox --box --small">
							<input type="checkbox" id="checkbox-item-box-01-2" name="checkbox-item-box-01-2" value="checkbox-value"></input>
							<label for="checkbox-item-box-01-2"></label>
						</div>
					</td>
					<td>Bryan</td>
					<td>Pineda</td> 
					<td class="xui-component-table_input">
						<div class="xui-form-input-group --small float-left">
							<input type="text" name="seek" value="25"></input>
							<button type="button" name="seek-button-first"><i class="lucide-chevron-up"></i></button>
							<button type="button" name="seek-button-last"><i class="lucide-chevron-down"></i></button>
						</div>
					</td>
					<td class="xui-component-table_action">
						<div class="xui-button --transparent --secondary xui-effect-ripple --icon --small --size-xy24-22 float-left">
							<i class="lucide-message-square-more"></i>
						</div>
					</td>
				</tr>
				<tr>
					<td class="xui-component-table_select">
						<div class="xui-form-checkbox --box --small">
							<input type="checkbox" id="checkbox-item-box-02-2" name="checkbox-item-box-02-2" value="checkbox-value"></input>
							<label for="checkbox-item-box-02-2"></label>
						</div>
					</td>
					<td>Alejandra</td>
					<td>Warren</td> 
					<td class="xui-component-table_input">
						<div class="xui-form-input-group --small float-left">
							<input type="text" name="seek" value="22"></input>
							<button type="button" name="seek-button-first"><i class="lucide-chevron-up"></i></button>
							<button type="button" name="seek-button-last"><i class="lucide-chevron-down"></i></button>
						</div>
					</td>
					<td class="xui-component-table_action">
						<div class="xui-button --transparent --secondary xui-effect-ripple --icon --small --size-xy24-22 float-left">
							<i class="lucide-message-square-more"></i>
						</div>
					</td>
				</tr>
				<tr>
					<td class="xui-component-table_select">
						<div class="xui-form-checkbox --box --small">
							<input type="checkbox" id="checkbox-item-box-03-2" name="checkbox-item-box-03-2" value="checkbox-value"></input>
							<label for="checkbox-item-box-03-2"></label>
						</div>
					</td>
					<td>Ruthie</td>
					<td>Mattera</td> 
					<td class="xui-component-table_input">
						<div class="xui-form-input-group --small float-left">
							<input type="text" name="seek" value="32"></input>
							<button type="button" name="seek-button-first"><i class="lucide-chevron-up"></i></button>
							<button type="button" name="seek-button-last"><i class="lucide-chevron-down"></i></button>
							</div>
					</td>
					<td class="xui-component-table_action">
						<div class="xui-button --transparent --secondary xui-effect-ripple --icon --small --size-xy24-22 float-left">
							<i class="lucide-message-square-more"></i>
						</div>
					</td>
				</tr>
				<tr>
					<td class="xui-component-table_select">
						<div class="xui-form-checkbox --box --small">
							<input type="checkbox" id="checkbox-item-box-04-2" name="checkbox-item-box-04-2" value="checkbox-value"></input>
							<label for="checkbox-item-box-04-2"></label>
						</div>
					</td>
					<td>Hailey</td>
					<td>Berke</td> 
					<td class="xui-component-table_input">
						<div class="xui-form-input-group --small float-left">
							<input type="text" name="seek" value="34"></input>
							<button type="button" name="seek-button-first"><i class="lucide-chevron-up"></i></button>
							<button type="button" name="seek-button-last"><i class="lucide-chevron-down"></i></button>
						</div>
					</td>
					<td class="xui-component-table_action">
						<div class="xui-button --transparent --secondary xui-effect-ripple --icon --small --size-xy24-22 float-left">
							<i class="lucide-message-square-more"></i>
						</div>
					</td>
				</tr>
				<tr>
					<td class="xui-component-table_select">
						<div class="xui-form-checkbox --box --small">
							<input type="checkbox" id="checkbox-item-box-05-2" name="checkbox-item-box-05-2" value="checkbox-value"></input>
							<label for="checkbox-item-box-05-2"></label>
						</div>
					</td>
					<td>Milo</td>
					<td>Tarnowski</td> 
					<td class="xui-component-table_input">
						<div class="xui-form-input-group --small float-left">
							<input type="text" name="seek" value="42"></input>
							<button type="button" name="seek-button-first"><i class="lucide-chevron-up"></i></button>
							<button type="button" name="seek-button-last"><i class="lucide-chevron-down"></i></button>
						</div>
					</td>
					<td class="xui-component-table_action">
						<div class="xui-button --transparent --secondary xui-effect-ripple --icon --small --size-xy24-22 float-left">
							<i class="lucide-message-square-more"></i>
						</div>
					</td>
				</tr>
			</tbody>
		</table>
	</div>
	<div class="xui-app-toolbar --left --compact">
		<div class="xui-app-toolbar_content">
			<div class="xui-form-input-group">
				<button type="button" name="seek-button-first"><i class="lucide-chevron-first"></i></i></button>
				<button type="button" name="seek-button-previous"><i class="lucide-chevron-left"></i></i></button>
				<input type="text" name="seek" value=""></input>
				<button type="button" name="seek-button-next"><i class="lucide-chevron-right"></i></button>
				<button type="button" name="seek-button-last"><i class="lucide-chevron-last"></i></i></button>
			</div>
			<div class="xui-button --transparent xui-effect-ripple --secondary --icon --size-xy30">
				<i class="lucide-wifi"></i>
			</div>
		</div>
	</div>
</div>
</form>
<!-- /item -->

</div>
