...
(function(){
	...
	this.getByClassNameAndAttributeValueFirstOne=function(el,className,attribute,attributeValue){
		var retV=this.getByClassNameAndAttributeValue(el,className,attribute,attributeValue);
		if(retV.length>0){
			return retV[0];	
		};
		return null;
	...
	this.getByTagNameAndAttributeValue=function(el,tagName,attribute,attributeValue){
		var retV=[];
	...
	this.getByTagNameAndAttributeValueFirstOne=function(el,tagName,attribute,attributeValue){
		var retV=this.getByTagNameAndAttributeValue(el,tagName,attribute,attributeValue);
		if(retV.length>0){
			return retV[0];	
		};
		return null;
	...
	this.getByTagNameAndClassName=function(el,tagName,className){
		var retV=[];
		var elList=this.getByClassName(el,className);
		for(var elIndex=0;elIndex<elList.length;++elIndex){
	...
	this.extractScript=function(inputHtmlAndScript){
		retV={
			js: "",
			html: ""
		};
